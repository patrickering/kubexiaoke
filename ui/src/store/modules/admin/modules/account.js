...
import {info, login, outLogin} from '@api/system/login';
            ...
            return new Promise((resolve, reject) => {
                ...
                login(user)
                    .then(async res => {
                        ...
                        dataUtils.setData(Setting.key.token, res.data)
                        if (res.state) {
                            info().then(async userRes => {
                                if (userRes.state) {
                                    dataUtils.setData(Setting.key.userInfo, userRes.data)
                                    // 设置 vuex 用户信息
                                    await dispatch('admin/user/set', userRes.data, {root: true});
                                    // 用户登录后从持久化数据加载一系列的设置
                                    await dispatch('load');

                                    getMenu().then(async menuRes => {
                                        dataUtils.setData(Setting.key.menu, menuRes.data)
                                        dataUtils.setData(Setting.key.menuActive, menuRes.data[0]);
                                        resolve(userRes);
                                    }).catch(err => {
                                        reject(err);
                                    })
